...
//Modifié le 29/12/2016 par Domochris.fr pour la CNC Root2
...
#define VERSION (0.3) //modif Domochris.fr
...
#define MAX_FEEDRATE (100.0) // vitesse maximale 100
#define MIN_FEEDRATE (0.1) // vitesse minimale 0.1
...
#define MM_PER_TURN 35.9 // mm par tour - fonction pas de vis - M6 : 1mm | M8 : 1.25mm
#define PULSE_US 5 // largeur du pulse moteur en µs

//Coefficient pour chaque axe Domochris.fr 29/12/2016
...
//-- params mouvements //modif Domochris.fr 28/12/16)
#define X_STEPS_PER_MM (STEPS_PER_TURN*MICROSTEPS_MODE/(MM_PER_TURN*CoefX))//rajout coéfissiant pour l'axe Y
//#define Y_STEPS_PER_MM (STEPS_PER_TURN*MICROSTEPS_MODE/MM_PER_TURN)
#define Y_STEPS_PER_MM (STEPS_PER_TURN*MICROSTEPS_MODE/(MM_PER_TURN*CoefY)) //rajout coéfissiant pour l'axe Y
#define Z_STEPS_PER_MM (STEPS_PER_TURN*MICROSTEPS_MODE/(MM_PER_TURN*CoefZ))//rajout coéfissiant pour l'axe Y


// limites mouvements en mm //modif Domochris.fr pour la cnc root 2
...
//-- vitesse
float fr=35.0; // memo feedrate en mm/sec - utilisée pour G01,  20.0
float frmax=35.0; // feedrate utilisé pour G00, 40.0
float currentfr=0; // le feedrate courant actif - different de fr pour initialisation possible, 0
...
void help() {
	...
	Serial.println(F("$ Modif Domochris.fr pour la cnc root 2 $"));
...
//rajout coef ca 29/12/2016
step_delay_us=((1000000/(nfr*X_STEPS_PER_MM))*CoefF); // calcul delai entre 2 pas, rajout coef ca 29/12/2016
